...
public class GcjcMainProcessTest extends CommonCheck {

    private static final ReportLog reportLog = new ReportLog(GcjcMainProcessTest.class);
    ...
    void initData() {
        CheckTypeEnum checkType = CheckTypeEnum.getCheckType(Constant.GCJC);
    ...
    void testTemp() {
        login("13988999991","a123456",true);
        chooseStage(cityName,stageName);
        enterModule("工程检查");
        waitLoading();
        clickInElementsByText(LocateType.ID,"cn.host.qc:id/titleTv","待整改");

        String checkName = "厨房-碗柜-不平";

        searchTargetTicketAndSubmitRecitify(checkName,"ABCDEFG",false);
    }

    @Test(priority = 1,description = " ======== >> 进入模块,断言tab展示工单数量是否与实际相符")
