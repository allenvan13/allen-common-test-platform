...
//@Test(description = "APP测试用例demo-注意removeTestData() testng调用方式,可调起其他测试用例且启动Spring 应用场景广泛 可用于测试前置、后续动作")
@Listeners({ExtentTestNGIReporterListener.class, AssertListener.class})
public class JxAndroidSummaryTestDemo extends AndroidTestBase {

    private static final ReportLog reportLog = new ReportLog(JxAndroidSummaryTestDemo.class);
    ...
    public void setUp() throws MalformedURLException {
        ...
        baseAndroidDriver = new AndroidDriver(new URL("http://127.0.0.1:4723/wd/hub"),capabilities);
    ...
    void testProjectBoardH5() {
        ...
        assertElementText(element,"南京紫樾府1标段");
    }
...
//        Assert.assertTrue(assertToastHasAppeared(baseAndroidDriver,10,"提交成功"),"测试失败! toast提示未出现或不正确");
    ...
    void testFhcySubmitProblem(@Optional("南京分户多楼层")String batchName,
        ...
        click(LocateType.ANDROID_UIAUTOMATOR,"new UiSelector().text(\"分户查验\")");
        clickText(batchName);
        clickText(banName);
        ...
        if (baseAndroidDriver!= null ) {
            ...
            if (existFlag) {
                try {
                    ...
                    baseAndroidDriver.findElement(By.id("cn.host.qc:id/title_bar_left_sub_iv")).click();
                }
