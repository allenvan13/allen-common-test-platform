...
public interface IOSCommon extends AppCommon{

    ReportLog reportLog = new ReportLog(IOSCommon.class);
     ...
     * @param second
     */
    default void setTimeOut(IOSDriver driver, int second) {
     ...
     * @return
     */
    default boolean isToastHasAppeared(IOSDriver driver, int outTimeSeconds, String message) {
     ...
     * @return
     */
    default void assertToastHasAppeared(IOSDriver driver, int outTimeSeconds, String message) {
    ...
    /**
     * 在屏幕上画图标记(滑动操作)
     * @param driver IOSDriver
     * @param type 1-勾 2-叉 3-圆形
     */
    default void drawMarkInScreen(IOSDriver driver,int type) {
     ...
     * @param steps
     */
    default void drawCircle (IOSDriver driver, Point origin, double radius, int steps) {
     ...
     *
     * @param driver IOSDriver对象
     * @param second 持续时间
     * @param num 滚动次数
     */
    default void swipeToDown(IOSDriver driver, double second, int num) {
     ...
     * @param num 滚动次数
     */
    default void swipeToUp(IOSDriver driver, double second, int num) {
        int nanos = (int) (second * 1000);
        Duration duration = Duration.ofNanos(nanos);
        int width = driver.manage().window().getSize().width;
        int height = driver.manage().window().getSize().height;
     ...
     * @param num 滑动次数
     */
    default void swipeToRight(IOSDriver driver, double second, int num) {
        int nanos = (int) (second * 1000);
        Duration duration = Duration.ofNanos(nanos);
        int width = driver.manage().window().getSize().width;
        int height = driver.manage().window().getSize().height;
     ...
     * @param num 滑动次数
     */
    default void swipeToLeft(IOSDriver driver, double second, int num) {
        int nanos = (int) (second * 1000);
        Duration duration = Duration.ofNanos(nanos);
        int width = driver.manage().window().getSize().width;
        int height = driver.manage().window().getSize().height;
     ...
     * @param pointY
     */
    default void enlargePoint(IOSDriver driver,double pointX,double pointY) {
     ...
     */
    default void swipeByDirect(IOSDriver driver, double ratioX, double ratioY, String direction){
     ...
     */
    default void swipeToPoint(IOSDriver driver, double startX, double startY, double moveToX, double moveToY){
     ...
     * @param maxY y坐标最大值
     */
    default PointOption clickRandomPointInCustomArea(IOSDriver driver, double minX, double minY, double maxX, double maxY) {
    ...
    default IOSDriver initBaseDriverAndStartService(AppiumServiceEntity serviceEntity, String iosInitInfos) {

        if (ObjectUtil.isEmpty(serviceEntity)) {
            throw new IllegalArgumentException("serviceEntity 不能为空!");
        }

        if (ObjectUtil.isEmpty(iosInitInfos)) {
            throw new IllegalArgumentException("iosInitInfos 配置信息不能为空!");
        }

        if (!CommonUtils.isJSONString(iosInitInfos)) {
            throw new IllegalArgumentException("iosInitInfos JSON 格式不合法! 请检查!");
        }

        DesiredCapabilities capabilities = new DesiredCapabilities();
        JSONObject initJson = JSON.parseObject(iosInitInfos);
     ...
     * @param driver IOSDriver
     */
    default void scaleScreen(IOSDriver driver) {
     ...
     * @param driver
     */
    default void enlargeScreen(IOSDriver driver) {
        MultiTouchAction multiTouchAction = new MultiTouchAction(driver);
        AndroidTouchAction actionA = new AndroidTouchAction(driver);
        AndroidTouchAction actionB = new AndroidTouchAction(driver);
        int width = driver.manage().window().getSize().width / 10;
    ...
    default void swipeDown(IOSDriver driver) {
        swipeToDown(driver,1,1);
    }

    default void swipeUp(IOSDriver driver) {
        swipeToUp(driver,1,1);
    }

    default void swipeLeft(IOSDriver driver) {
        swipeToLeft(driver,1,1);
    }

    default void swipeRight(IOSDriver driver) {
     ...
     */
    default void assertToastHasAppeared(IOSDriver driver, String excelInputText) {
    ...
    default void clickText(IOSDriver driver, String excelInputText) {
    ...
    default void drawMarkInScreen(IOSDriver driver,String excelInputText) {
        if (ObjectUtil.isEmpty(excelInputText)) {
    ...
    default void clickRandomPointInCustomArea(IOSDriver driver,String excelInputText) {
        if (ObjectUtil.isEmpty(excelInputText)) {
    ...
    default void clickPoint(IOSDriver driver,String excelInputText) {
        if (ObjectUtil.isEmpty(excelInputText)) {
    ...
    default void setTimeOut(IOSDriver driver,String excelInputText) {
        if (ObjectUtil.isEmpty(excelInputText)) {
            throw new IllegalArgumentException("clickPoint 输入值坐标值不能为空! 格式: X坐标,Y坐标  例如 100,200");
        }
